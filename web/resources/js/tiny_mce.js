...
tinymce.util.Quirks=function(d){
    ...
    var E;
    E=F.keyCode==s;
    if(!F.isDefaultPrevented()&&(E||F.keyCode==r)&&!l.modifierPressed(F)){
        F.preventDefault();
        C(E)
        }
    });
...
function B(){
    function D(F,I){
        ...
        if(E.nodeType==1&&E.hasChildNodes()){
            E=E.childNodes[Math.min(I?H:(H>0?H-1:0),E.childNodes.length-1)]
            }
            return E
        }
        function C(H,L){
        ...
        if(G.nodeType==3&&((L&&K>0)||(!L&&K<G.nodeValue.length))){
        return false
        }while(G!==F){
    ...
    if(!H.isDefaultPrevented()&&(G==s||G==r)){
        E=A.getRng(true);
        if(C(E,true)&&C(E,false)&&(E.collapsed||o.findCommonAncestor(D(E,true),D(E))===o.getRoot())){
            F.setContent("");
            F.nodeChanged();
            H.preventDefault()
            }
...
function p(){
    d.onKeyDown.add(function(I,J){
        var H,C,D,F,G,K,E;
        H=J.keyCode==s;
        if(!J.isDefaultPrevented()&&(H||J.keyCode==r)&&!l.modifierPressed(J)){
            C=A.getRng();
            D=C.startContainer;
            F=C.startOffset;
            E=C.collapsed;
...
function e(){
    d.onKeyDown.add(function(G,H){
        ...
        if(!E.collapsed||I!==0){
            return
        }while(F&&F.parentNode&&F.parentNode.firstChild==F&&F.parentNode!=C){
            F=F.parentNode
            }
...
function j(){
    d.onKeyDown.add(function(C,D){
        if(!D.isDefaultPrevented()&&D.keyCode==8&&A.getNode().nodeName=="IMG"){
            D.preventDefault();
...
if(G.render_ui&&F.theme){
    ...
    if(p.editorContainer.nodeType){
        p.editorContainer=p.editorContainer.id=p.editorContainer.id||F.id+"_parent"
        }
        if(p.iframeContainer.nodeType){
        p.iframeContainer=p.iframeContainer.id=p.iframeContainer.id||F.id+"_iframecontainer"
        }
        y=p.iframeHeight||B.offsetHeight
    }
...
if(p.editorContainer){
    ...
    B.style.visibility=F.orgVisibility;
...
n.parser.addAttributeFilter("src,href,style",function(s,t){
    ...
    while(u--){
        ...
        if(!x.attributes.map[v]){
            if(t==="style"){
                x.attr(v,z.serializeStyle(z.parseStyle(y),x.name))
                }else{
                x.attr(v,n.convertURL(y,t,x.name))
                }
            }
    }
});
n.parser.addNodeFilter("script",function(s,t){
    var u=s.length,v;
    while(u--){
        v=s[u];
        v.attr("type","mce-"+(v.attr("type")||"text/javascript"))
        }
    });
...
n.parser.addNodeFilter("p,h1,h2,h3,h4,h5,h6,div",function(t,u){
    var v=t.length,x,s=n.schema.getNonEmptyElements();
    while(v--){
        x=t[v];
        if(x.isEmpty(s)){
            x.empty().append(new k.html.Node("br",1)).shortEnded=true
            }
        }
});
...
if(p.directionality){
    ...
    n.onBeforeSetContent.add(function(s,t){
        i(p.protect,function(u){
            t.content=t.content.replace(u,function(v){
                return"<!--mce:protected "+escape(v)+"-->"
                })
            })
        })
    }
...
if(p.padd_empty_editor){
    n.onPostProcess.add(function(s,t){
...
(function(a){
    a.Formatter=function(Y){
        var O={},R=a.each,c=Y.dom,r=Y.selection,t=a.dom.TreeWalker,M=new a.dom.RangeUtils(c),d=Y.schema.isValidChild,H=c.isBlock,m=Y.settings.forced_root_block,s=c.nodeIndex,G=a.isGecko?"\u200B":"\uFEFF",e=/^(src|href|style)$/,V=false,C=true,D,x=c.getContentEditable;
        function A(Z){
            return Z instanceof Array
            }
            function n(aa,Z){
            return c.getParents(aa,Z,c.getRoot())
            }
            function b(Z){
            return Z.nodeType===1&&Z.id==="_mce_caret"
            }
            function j(){
            l({
                alignleft:[{
...
link:{
    ...
    onformat:function(ab,Z,aa){
        R(aa,function(ad,ac){
            c.setAttrib(ab,ac,ad)
            })
        }
    },
removeformat:[{
...
R("p h1 h2 h3 h4 h5 h6 div address pre div code dt dd samp".split(/\s/),function(Z){
    l(Z,{
        block:Z,
        remove:"all"
    })
    });
l(Y.settings.formats)
}
function U(){
    Y.addShortcut("ctrl+b","bold_desc","Bold");
    Y.addShortcut("ctrl+i","italic_desc","Italic");
    Y.addShortcut("ctrl+u","underline_desc","Underline");
    for(var Z=1;Z<=6;Z++){
        Y.addShortcut("ctrl+"+Z,"",["FormatBlock",false,"h"+Z])
        }
        Y.addShortcut("ctrl+7","",["FormatBlock",false,"p"]);
    Y.addShortcut("ctrl+8","",["FormatBlock",false,"div"]);
    Y.addShortcut("ctrl+9","",["FormatBlock",false,"address"])
    }
    function T(Z){
    return Z?O[Z]:O
    }
        ...
        if(typeof(Z)!=="string"){
            ...
            R(aa,function(ab){
                if(ab.deep===D){
                    ...
                    if(ab.split===D){
                    ab.split=!ab.selector||ab.inline
                    }
                    if(ab.remove===D&&ab.selector&&!ab.inline){
                    ab.remove="none"
                    }
                    if(ab.selector&&ab.inline){
                    ab.mixed=true;
                    ab.block_expand=true
                    }
                    if(typeof(ab.classes)==="string"){
                    ab.classes=ab.classes.split(/\s+/)
                    }
                });
        O[Z]=aa
        }
...
var i=function(aa){
    var Z;
    Y.dom.getParent(aa,function(ab){
        Z=Y.dom.getStyle(ab,"text-decoration");
        return Z&&Z!=="none"
        });
    return Z
    };
...
var K=function(Z){
    var aa;
    if(Z.nodeType===1&&Z.parentNode&&Z.parentNode.nodeType===1){
        aa=i(Z.parentNode);
        if(Y.dom.getStyle(Z,"color")&&aa){
            Y.dom.setStyle(Z,"text-decoration",aa)
            }else{
            if(Y.dom.getStyle(Z,"textdecoration")===aa){
                Y.dom.setStyle(Z,"text-decoration",null)
                }
            }
    }
};
...
function W(ac,aj,ae){
    var af=T(ac),ak=af[0],ai,aa,ah,ag=r.isCollapsed();
    function Z(ao,an){
        an=an||ak;
        if(ao){
            if(an.onformat){
                an.onformat(ao,an,aj,ae)
                }
                R(an.styles,function(aq,ap){
                c.setStyle(ao,ap,q(aq,aj))
                });
            R(an.attributes,function(aq,ap){
                c.setAttrib(ao,ap,q(aq,aj))
                });
            R(an.classes,function(ap){
                ap=q(ap,aj);
                if(!c.hasClass(ao,ap)){
                    c.addClass(ao,ap)
                    }
...
function ad(){
    function ap(aw,au){
        var av=new t(au);
        for(ae=av.current();ae;ae=av.prev()){
            if(ae.childNodes.length>1||ae==aw||ae.tagName=="BR"){
                return ae
                }
            }
        }
    var ao=Y.selection.getRng();
var at=ao.startContainer;
var an=ao.endContainer;
if(at!=an&&ao.endOffset===0){
    var ar=ap(at,an);
    var aq=ar.nodeType==3?ar.length:ar.childNodes.length;
    ao.setEnd(ar,aq)
    }
    return ao
}
function ab(aq,aw,au,at,ao){
    var an=[],ap=-1,av,ay=-1,ar=-1,ax;
    R(aq.childNodes,function(aA,az){
        if(aA.nodeName==="UL"||aA.nodeName==="OL"){
            ap=az;
            av=aA;
            return false
            }
...
R(aq.childNodes,function(aA,az){
    if(aA.nodeName==="SPAN"&&c.getAttrib(aA,"data-mce-type")=="bookmark"){
        if(aA.id==aw.id+"_start"){
            ay=az
            }else{
            if(aA.id==aw.id+"_end"){
                ar=az
...
if(ap<=0||(ay<ap&&ar>ap)){
    R(a.grep(aq.childNodes),ao);
    return 0
    }else{
    ax=c.clone(au,V);
    R(a.grep(aq.childNodes),function(aA,az){
        if((ay<ap&&az<ap)||(ay>ap&&az>ap)){
            an.push(aA);
            aA.parentNode.removeChild(aA)
            }
        });
if(ay<ap){
    aq.insertBefore(ax,av)
    }else{
    if(ay>ap){
        aq.insertBefore(ax,av.nextSibling)
        }
    }
at.push(ax);
R(an,function(az){
...
function al(ao,aq,au){
    var an=[],at,ap,ar=true;
    at=ak.inline||ak.block;
    ap=c.create(at);
    Z(ap);
    M.walk(ao,function(av){
        var aw;
        function ax(ay){
            var aD,aB,az,aA,aC;
            aC=ar;
            aD=ay.nodeName.toLowerCase();
            aB=ay.parentNode.nodeName.toLowerCase();
            if(ay.nodeType===1&&x(ay)){
                aC=ar;
                ar=x(ay)==="true";
                aA=true
                }
            ...
            if(ak.wrapper&&y(ay,ac,aj)){
                aw=0;
                return
            }
            if(ar&&!aA&&ak.block&&!ak.wrapper&&I(aD)){
                ay=c.rename(ay,at);
                Z(ay);
                an.push(ay);
                aw=0;
            ...
            if(ak.selector){
                R(af,function(aE){
                    if("collapsed" in aE&&aE.collapsed!==ag){
                        return
                    }
                    if(c.is(ay,aE.selector)&&!b(ay)){
                        Z(ay,aE);
                        az=true
                        }
                    });
        ...
        if(ar&&!aA&&d(at,aD)&&d(aB,at)&&!(!au&&ay.nodeType===3&&ay.nodeValue.length===1&&ay.nodeValue.charCodeAt(0)===65279)&&!b(ay)){
        if(!aw){
            aw=c.clone(ap,V);
            ay.parentNode.insertBefore(aw,ay);
            an.push(aw)
            }
            aw.appendChild(ay)
        }else{
        if(aD=="li"&&aq){
            aw=ab(ay,aq,ap,an,ax)
            }else{
            aw=0;
            R(a.grep(ay.childNodes),ax);
            if(aA){
                ar=aC
                }
                aw=0
...
if(ak.wrap_links===false){
    R(an,function(av){
        function aw(aA){
            ...
            if(aA.nodeName==="A"){
                ay=c.clone(ap,V);
                an.push(ay);
                ax=a.grep(aA.childNodes);
                for(az=0;az<ax.length;az++){
                    ay.appendChild(ax[az])
                    }
                    aA.appendChild(ay)
                }
                R(a.grep(aA.childNodes),aw)
            }
            aw(av)
        })
    }
    ...
    function aw(az){
    var aB,aA;
    R(az.childNodes,function(aC){
        if(aC.nodeType==1&&!J(aC)&&!b(aC)){
            aB=aC;
            return V
            }
        });
...
av=ay(ax);
if((an.length>1||!H(ax))&&av===0){
    c.remove(ax,1);
    return
}
if(ak.inline||ak.wrapper){
    if(!ak.exact&&av===1){
        ax=aw(ax)
        }
        R(af,function(az){
        R(c.select(az.inline,ax),function(aB){
            var aA;
            if(az.wrap_links===false){
                aA=aB.parentNode;
                do{
                    if(aA.nodeName==="A"){
                        return
        ...
        X(az,aj,aB,az.exact?aB:null)
            })
    });
if(y(ax.parentNode,ac,aj)){
    c.remove(ax,1);
    ax=0;
    return C
    }
    if(ak.merge_with_parents){
    c.getParent(ax.parentNode,function(az){
        if(y(az,ac,aj)){
            c.remove(ax,1);
            ax=0;
            return C
...
if(ak){
    if(ae){
        if(ae.nodeType){
            aa=c.createRng();
            aa.setStartBefore(ae);
            aa.setEndAfter(ae);
            al(p(aa,af),null,true)
            }else{
            al(ae,null,true)
            }
        }else{
    if(!ag||!ak.inline||c.select("td.mceSelected,th.mceSelected").length){
        var am=Y.selection.getNode();
        if(!m&&af[0].defaultBlock&&!c.getParent(am,c.isBlock)){
            W(af[0].defaultBlock)
            }
            Y.selection.setRng(ad());
        ai=r.getBookmark();
        al(p(r.getRng(C),af),ai);
        if(ak.styles&&(ak.styles.color||ak.styles.textDecoration)){
            a.walk(am,K,"childNodes");
            K(am)
            }
            r.moveToBookmark(ai);
        P(r.getRng(C));
...
function B(ab,ak,ad){
    var ae=T(ab),am=ae[0],ai,ah,aa,aj=true;
    function ac(at){
        var ar,aq,ap,ao,av,au;
        if(at.nodeType===1&&x(at)){
            av=aj;
            aj=x(at)==="true";
            au=true
            }
            ar=a.grep(at.childNodes);
        if(aj&&!au){
            for(aq=0,ap=ae.length;aq<ap;aq++){
                if(X(ae[aq],ak,at,at)){
                    break
                }
            }
        ...
        if(am.deep){
    if(ar.length){
        for(aq=0,ap=ar.length;aq<ap;aq++){
            ac(ar[aq])
            }
            if(au){
            aj=av
...
function af(ao){
    var ap;
    R(n(ao.parentNode).reverse(),function(aq){
        var ar;
        if(!ap&&aq.id!="_start"&&aq.id!="_end"){
            ar=y(aq,ab,ak);
            if(ar&&ar.split!==false){
                ap=aq
                }
            }
    });
...
function Z(ar,ao,au,ax){
    var ay,aw,av,aq,at,ap;
    if(ar){
        ap=ar.parentNode;
        for(ay=ao.parentNode;ay&&ay!=ap;ay=ay.parentNode){
            aw=c.clone(ay,V);
            for(at=0;at<ae.length;at++){
                if(X(ae[at],ak,aw,aw)){
                    aw=0;
                    break
                }
    ...
    if(av){
    au.parentNode.insertBefore(av,au);
    aq.appendChild(au)
    }
}
return ao
...
function al(ao){
    return Z(af(ao),ao,ao,true)
    }
    function ag(aq){
    var ap=c.get(aq?"_start":"_end"),ao=ap[aq?"firstChild":"lastChild"];
    if(J(ao)){
        ao=ao[aq?"firstChild":"lastChild"]
        }
        c.remove(ap,true);
    return ao
    }
    ...
    if(am.split){
        ...
        ar=L(ao);
        if(aq!=ar){
            if(/^(TR|TD)$/.test(aq.nodeName)&&aq.firstChild){
                aq=(aq.nodeName=="TD"?aq.firstChild:aq.firstChild.firstChild)||aq
                }
                aq=Q(aq,"span",{
                id:"_start",
                "data-mce-type":"bookmark"
            ...
            ao.startContainer=aq.parentNode;
        ao.startOffset=s(aq);
        ao.endContainer=ar.parentNode;
        ao.endOffset=s(ar)+1
        }
        M.walk(ao,function(at){
        R(at,function(au){
            ac(au);
            if(au.nodeType===1&&Y.dom.getStyle(au,"text-decoration")==="underline"&&au.parentNode&&i(au.parentNode)==="underline"){
                X({
                    deep:false,
                    exact:true,
                    inline:"span",
...
if(!r.isCollapsed()||!am.inline||c.select("td.mceSelected,th.mceSelected").length){
    ai=r.getBookmark();
    an(r.getRng(C));
    r.moveToBookmark(ai);
    if(am.inline&&k(ab,ak,r.getStart())){
        P(r.getRng(true))
        }
        Y.nodeChanged()
    }else{
...
function F(aa,ac,ab){
    var Z=T(aa);
    if(k(aa,ac,ab)&&(!("toggle" in Z[0])||Z[0].toggle)){
        B(aa,ac,ab)
        }else{
        W(aa,ac,ab)
        }
    }
function y(aa,Z,af,ad){
    var ab=T(Z),ag,ae,ac;
    function ah(al,an,ao){
        var ak,am,ai=an[ao],aj;
        if(an.onmatch){
            return an.onmatch(al,an,ao)
            }
            if(ai){
            if(ai.length===D){
                for(ak in ai){
                    if(ai.hasOwnProperty(ak)){
                        if(ao==="attributes"){
                            am=c.getAttrib(al,ak)
                            }else{
                            am=N(al,ak)
                            }
                            if(ad&&!am&&!an.exact){
                        ...
                        if((!ad||an.exact)&&!g(am,q(ai[ak],af))){
                            return
                        }
                    }
                }
    ...
    for(aj=0;aj<ai.length;aj++){
        if(ao==="attributes"?c.getAttrib(al,ai[aj]):N(al,ai[aj])){
            return an
            }
        }
    }
...
if(ab&&aa){
    for(ae=0;ae<ab.length;ae++){
        ag=ab[ae];
        if(h(aa,ag)&&ah(aa,ag,"attributes")&&ah(aa,ag,"styles")){
            if(ac=ag.classes){
                for(ae=0;ae<ac.length;ae++){
                    if(!c.hasClass(aa,ac[ae])){
                        return
                    }
                }
                }
...
function k(ab,ad,ac){
    var aa;
    function Z(ae){
        ae=c.getParent(ae,function(af){
            return !!y(af,ab,ad,true)
            });
        return y(ae,ab,ad)
        }
        if(ac){
...
function v(ag,af){
    var ad,ae=[],ac={},ab,aa,Z;
    ad=r.getStart();
    c.getParent(ad,function(aj){
        var ai,ah;
        for(ai=0;ai<ag.length;ai++){
            ah=ag[ai];
            if(!ac[ah]&&y(aj,ah,af)){
                ac[ah]=true;
                ae.push(ah)
...
function g(aa,Z){
    ...
    aa=""+(aa.nodeName||aa);
    Z=""+(Z.nodeName||Z);
    return aa.toLowerCase()==Z.toLowerCase()
    }
    function N(aa,Z){
    var ab=c.getStyle(aa,Z);
    if(Z=="color"||Z=="backgroundColor"){
        ab=c.toHex(ab)
        }
        if(Z=="fontWeight"&&ab==700){
        ab="bold"
        }
        return""+ab
    }
    ...
    if(typeof(Z)!="string"){
        ...
        if(aa){
            Z=Z.replace(/%(\w+)/g,function(ac,ab){
                return aa[ab]||ac
                })
            }
        }
...
function f(Z){
    return Z&&Z.nodeType===3&&/^([\t \r\n]+|)$/.test(Z.nodeValue)
    }
    function Q(ab,aa,Z){
    var ac=c.create(aa,Z);
    ab.parentNode.insertBefore(ac,ab);
    ac.appendChild(ab);
    return ac
    }
    function p(Z,ak,ac){
    var an,al,af,aj,ab=Z.startContainer,ag=Z.startOffset,ap=Z.endContainer,ai=Z.endOffset;
    function am(ax){
        var ar,av,aw,au,at,aq;
        ar=av=ax?ab:ap;
        at=ax?"previousSibling":"nextSibling";
        aq=c.getRoot();
        if(ar.nodeType==3&&!f(ar)){
            if(ax?ag>0:ai<ar.nodeValue.length){
                return ar
                }
            }
        for(;;){
        if(!ak[0].block_expand&&H(av)){
            return av
            }
            for(au=av[at];au;au=au[at]){
            if(!J(au)&&!f(au)){
                return av
                }
            }
...
function ae(aq,ar){
    if(ar===D){
        ar=aq.nodeType===3?aq.length:aq.childNodes.length
        }while(aq&&aq.hasChildNodes()){
        aq=aq.childNodes[ar];
        if(aq){
            ar=aq.nodeType===3?aq.length:aq.childNodes.length
            }
...
if(ab.nodeType==1&&ab.hasChildNodes()){
    al=ab.childNodes.length-1;
    ab=ab.childNodes[ag>al?al:ag];
    if(ab.nodeType==3){
        ag=0
        }
    }
if(ap.nodeType==1&&ap.hasChildNodes()){
    al=ap.childNodes.length-1;
    ap=ap.childNodes[ai>al?al:ai-1];
    if(ap.nodeType==3){
        ai=ap.nodeValue.length
        }
    }
function ao(ar){
    var aq=ar;
    while(aq){
        if(aq.nodeType===1&&x(aq)){
            return x(aq)==="false"?aq:ar
            }
            aq=aq.parentNode
        }
        return ar
    }
    function ah(ar,aw,ay){
    var av,at,ax,aq;
    function au(aA,aC){
        var aD,az,aB=aA.nodeValue;
        if(typeof(aC)=="undefined"){
            aC=ay?aB.length:0
            }
            if(ay){
            aD=aB.lastIndexOf(" ",aC);
            az=aB.lastIndexOf("\u00a0",aC);
            aD=aD>az?aD:az;
            if(aD!==-1&&!ac){
                aD++
            }
...
av=new t(ar,c.getParent(ar,H)||Y.getBody());
while(at=av[ay?"prev":"next"]()){
    if(at.nodeType===3){
        aq=at;
        ax=au(at);
        if(ax!==-1){
...
function ad(ar,aq){
    var at,au,aw,av;
    if(ar.nodeType==3&&ar.nodeValue.length===0&&ar[aq]){
        ar=ar[aq]
        }
        at=n(ar);
    for(au=0;au<at.length;au++){
        for(aw=0;aw<ak.length;aw++){
            av=ak[aw];
            if("collapsed" in av&&av.collapsed!==Z.collapsed){
                continue
            }
            if(c.is(at[au],av.selector)){
                return at[au]
                }
            }
        }
...
function aa(ar,aq,au){
    var at;
    if(!ak[0].wrapper){
        at=c.getParent(ar,ak[0].block)
        }
        if(!at){
        at=c.getParent(ar.nodeType==3?ar.parentNode:ar,H)
        }
        if(at&&ak[0].wrapper){
        at=n(at,"ul,ol").reverse()[0]||at
        }
        if(!at){
        at=ar;
        while(at[aq]&&!H(at[aq])){
...
ab=ao(ab);
ap=ao(ap);
if(J(ab.parentNode)||J(ab)){
    ab=J(ab)?ab:ab.parentNode;
    ab=ab.nextSibling||ab;
    if(ab.nodeType==3){
        ag=0
        }
    }
if(J(ap.parentNode)||J(ap)){
    ap=J(ap)?ap:ap.parentNode;
    ap=ap.previousSibling||ap;
    if(ap.nodeType==3){
        ai=ap.length
        }
    }
if(ak[0].inline){
    ...
    af=ae(ap,ai);
if(af.node){
    while(af.node&&af.offset===0&&af.node.previousSibling){
        af=ae(af.node.previousSibling)
        }
        if(af.node&&af.offset>0&&af.node.nodeType===3&&af.node.nodeValue.charAt(af.offset-1)===" "){
        if(af.offset>1){
            ap=af.node;
            ap.splitText(af.offset-1)
            }
        }
}
}
if(ak[0].inline||ak[0].block_expand){
    if(!ak[0].inline||(ab.nodeType!=3||ag===0)){
        ab=am(true)
        }
        if(!ak[0].inline||(ap.nodeType!=3||ai===ap.nodeValue.length)){
        ap=am()
        }
    }
if(ak[0].selector&&ak[0].expand!==V&&!ak[0].inline){
    ab=ad(ab,"previousSibling");
    ap=ad(ap,"nextSibling")
    }
    if(ak[0].block||ak[0].selector){
    ab=aa(ab,"previousSibling");
    ap=aa(ap,"nextSibling");
    if(ak[0].block){
        if(!H(ab)){
            ab=am(true)
            }
...
function X(af,ae,ac,Z){
    var ab,aa,ad;
    if(!h(ac,af)){
        return V
        }
        if(af.remove!="all"){
        R(af.styles,function(ah,ag){
            ah=q(ah,ae);
            if(typeof(ag)==="number"){
                ag=ah;
                Z=0
        ...
        if(ad&&c.getAttrib(ac,"style")==""){
            ac.removeAttribute("style");
            ac.removeAttribute("data-mce-style")
            }
            R(af.attributes,function(ai,ag){
            var ah;
            ai=q(ai,ae);
            if(typeof(ag)==="number"){
                ag=ai;
                ...
                if(!Z||g(c.getAttrib(Z,ag),ai)){
                if(ag=="class"){
                    ai=c.getAttrib(ac,ag);
                    if(ai){
                        ah="";
                        R(ai.split(/\s+/),function(aj){
                            if(/mce\w+/.test(aj)){
                                ah+=(ah?" ":"")+aj
                                }
                            });
                    if(ah){
                        c.setAttrib(ac,ag,ah);
                        return
                    }
                }
            }
        if(ag=="class"){
            ac.removeAttribute("className")
            }
            if(e.test(ag)){
            ac.removeAttribute("data-mce-"+ag)
            }
            ac.removeAttribute(ag)
        }
    });
R(af.classes,function(ag){
    ag=q(ag,ae);
    if(!Z||c.hasClass(Z,ag)){
        c.removeClass(ac,ag)
        }
    });
aa=c.getAttribs(ac);
for(ab=0;ab<aa.length;ab++){
    if(aa[ab].nodeName.indexOf("_")!==0){
        return V
        }
    }
}
...
function o(ab,ac){
    var Z=ab.parentNode,aa;
    function ad(af,ae,ag){
        af=E(af,ae,ag);
        return !af||(af.nodeName=="BR"||H(af))
        }
        if(ac.block){
        if(!m){
            if(H(ab)&&!H(Z)){
                if(!ad(ab,V)&&!ad(ab.firstChild,C,1)){
                    ab.insertBefore(c.create("br"),ab.firstChild)
                    }
                    if(!ad(ab,C)&&!ad(ab.lastChild,V,1)){
                    ab.appendChild(c.create("br"))
                    }
                }
        }else{
    if(Z==c.getRoot()){
        if(!ac.list_block||!g(ab,ac.list_block)){
            R(a.grep(ab.childNodes),function(ae){
                if(d(m,ae.nodeName.toLowerCase())){
                    if(!aa){
                        aa=Q(ae,m)
                        }else{
                        aa.appendChild(ae)
...
if(ac.selector&&ac.inline&&!g(ac.inline,ab)){
    return
}
c.remove(ab,1)
}
function E(aa,Z,ab){
    if(aa){
        Z=Z?"nextSibling":"previousSibling";
        for(aa=ab?aa:aa[Z];aa;aa=aa[Z]){
            if(aa.nodeType==1||!f(aa)){
                return aa
                }
            }
        }
...
function J(Z){
    return Z&&Z.nodeType==1&&Z.getAttribute("data-mce-type")=="bookmark"
    }
    function u(ad,ac){
    var Z,ab,aa;
    function af(ai,ah){
        if(ai.nodeName!=ah.nodeName){
            return V
            }
            function ag(ak){
            var al={};
            
            R(c.getAttribs(ak),function(am){
                var an=am.nodeName.toLowerCase();
                if(an.indexOf("_")!==0&&an!=="style"){
                    al[an]=c.getAttrib(ak,an)
                    }
                });
        return al
        }
...
if(!aj(ag(ai),ag(ah))){
    ...
    if(!aj(c.parseStyle(c.getAttrib(ai,"style")),c.parseStyle(c.getAttrib(ah,"style")))){
    return V
    }
    return C
}
function ae(ah,ag){
    for(ab=ah;ab;ab=ab[ag]){
        if(ab.nodeType==3&&ab.nodeValue.length!==0){
            return ah
            }
            if(ab.nodeType==1&&!J(ab)){
            return ab
            }
        }
    return ah
...
if(ad&&ac){
    ad=ae(ad,"previousSibling");
    ac=ae(ac,"nextSibling");
    if(af(ad,ac)){
        for(ab=ad.nextSibling;ab&&ab!=ac;){
            aa=ab;
            ab=ab.nextSibling;
            ad.appendChild(aa)
            }
            c.remove(ac);
        R(a.grep(ac.childNodes),function(ag){
            ad.appendChild(ag)
            });
        return ad
        }
...
function I(Z){
    return/^(h[1-6]|p|div|pre|address|dl|dt|dd)$/.test(Z)
    }
    function L(aa,ae){
    var Z,ad,ab,ac;
    Z=aa[ae?"startContainer":"endContainer"];
    ad=aa[ae?"startOffset":"endOffset"];
    if(Z.nodeType==1){
        ab=Z.childNodes.length-1;
        if(!ae&&ad){
            ad--
        }
        Z=Z.childNodes[ad>ab?ab:ad]
        }
        if(Z.nodeType===3&&ae&&ad>=Z.nodeValue.length){
        Z=new t(Z,Y.getBody()).next()||Z
        }
        if(Z.nodeType===3&&!ae&&ad===0){
        Z=new t(Z,Y.getBody()).prev()||Z
        }
        return Z
    }
    function S(ai,Z,ag){
    var aj="_mce_caret",aa=Y.settings.caret_debug;
    function ab(an){
        ...
        if(an){
            am.appendChild(Y.getDoc().createTextNode(G))
            }
            return am
        }
        function ah(an,am){
        while(an){
            if((an.nodeType===3&&an.nodeValue!==G)||an.childNodes.length>1){
                return false
                }
                if(am&&an.nodeType===1){
                am.push(an)
...
function ak(){
    ...
    ao=an;
    if(an.nodeType==3){
        if(ap!=an.nodeValue.length||an.nodeValue===G){
            ar=true
            }
            ao=ao.parentNode
        }while(ao){
    ...
    if(ar){
        ...
        for(aq=av.length-1;aq>=0;aq--){
            ...
            ao.appendChild(c.doc.createTextNode(G));
        ao=ao.firstChild;
        c.insertAfter(au,aw);
...
function al(){
    ...
    am=ae(r.getStart());
    if(am&&!c.isEmpty(am)){
        a.walk(am,function(ap){
            if(ap.nodeType==1&&ap.id!==aj&&!c.isEmpty(ap)){
                c.setAttrib(ap,"data-mce-bogus",null)
                }
            },"childNodes")
    }
}
if(!self._hasCaretEvents){
    Y.onBeforeGetContent.addToTop(function(){
        var am=[],an;
        if(ah(ae(r.getStart()),am)){
            an=am.length;
            while(an--){
...
a.each("onMouseUp onKeyUp".split(" "),function(am){
    Y[am].addToTop(function(){
        ac();
        al()
        })
    });
Y.onKeyDown.addToTop(function(am,ao){
    var an=ao.keyCode;
    if(an==8||an==37||an==39){
        ac(ae(r.getStart()))
        }
...
function P(aa){
    var Z=aa.startContainer,ag=aa.startOffset,ac,af,ae,ab,ad;
    if(Z.nodeType==3&&ag>=Z.nodeValue.length){
        ag=s(Z);
        Z=Z.parentNode;
        ac=true
        }
        if(Z.nodeType==1){
        ab=Z.childNodes;
        Z=ab[Math.min(ag,ab.length-1)];
        af=new t(Z,c.getParent(Z,c.isBlock));
        if(ag>ab.length-1||ac){
            af.next()
            }
            for(ae=af.current();ae;ae=af.next()){
            if(ae.nodeType==3&&!f(ae)){
                ad=c.create("a",null,G);
                ae.parentNode.insertBefore(ad,ae);
                aa.setStart(ae,0);
                r.setRng(aa);
                c.remove(ad);
                return
...
(function(b){
    ...
    b.EnterKey=function(e){
        ...
        function f(y){
            ...
            function B(I){
                return I&&h.isBlock(I)&&!/^(TD|TH|CAPTION)$/.test(I.nodeName)&&!/^(fixed|absolute)/i.test(I.style.position)&&h.getContentEditable(I)!=="true"
                }
                function m(J){
                var O,M,I,P,N,L=J,K;
                I=h.createRng();
...
function q(J){
    ...
    if(c.keep_styles!==false){
        do{
            if(/^(SPAN|STRONG|B|EM|I|FONT|STRIKE|U)$/.test(K.nodeName)){
                L=K.cloneNode(false);
                h.setAttrib(L,"id","");
                if(M.hasChildNodes()){
...
function p(L){
    ...
    if(x.nodeType==3&&(L?t>0:t<x.nodeValue.length)){
        ...
        if(x.nodeName==="TABLE"||(x.previousSibling&&x.previousSibling.nodeName=="TABLE")){
        return(z&&!L)||(!z&&L)
        }
        K=new a(x,o);
